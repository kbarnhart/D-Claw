...
def pointfromfort(point,solutionlist):
    ...
    dintersection = (xp>=griddict['xlowdomain'])&(xp<=griddict['xhidomain'])& \
    ...
    for trygrid in solutionlist:
        intersection = (xp>=trygrid['xlow'])&(xp<=trygrid['xupper'])& \
                        (yp>=trygrid['ylow'])&(yp<=trygrid['yupper'])
        if intersection:
    ...
    try:
        ...
        for trygrid in solutionlist:
            intersection = (xp+eps>=trygrid['xlow'])&(xp-eps<=trygrid['xupper'])& \
                        (yp+eps>=trygrid['ylow'])&(yp-eps<=trygrid['yupper'])
    ...
    q = qll*(xr-xp)*(yu-yp) + qlr*(xp-xl)*(yu-yp) + qul*(xr-xp)*(yp-yl) + qur*(xp-xl)*(yp-yl)
    q = q/(dx*dy)
